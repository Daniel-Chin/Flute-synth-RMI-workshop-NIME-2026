'''
Could not apply -Wundef to Just My Code, hence this.  
'''
...
def remove_guard():
    for filename in os.listdir(PROJECT_PATH):
        ...
        with open(full_filename, 'r') as f:
            for line in f:
                lines.append(line)
                if 'I am using syntax error to denote undefined macro here' in line:
                    lines.pop()
                    assert 'ifndef' in lines.pop()
                    assert 'endif' in next(f)
        with open(full_filename, 'w', newline='') as f:
            for line in lines:
                f.write(line)
    print('remove_guard() ok')
